...
pub struct SudoParams {
    ...
    pub current_bid_id: u64,
    pub current_collection_bid_id: u64,
}
...
/// Primary key for bids: (collection, token_id, bid_id)
pub type BidKey = (Addr, TokenId, BidId);
/// Convenience bid key constructor
pub fn bid_key(collection: &Addr, token_id: TokenId, bid_id: BidId) -> BidKey {
    (collection.clone(), token_id, bid_id)
}
...
pub struct BidIndicies<'a> {
    ...
    pub bid_id: MultiIndex<'a, BidId, Bid, BidKey>, // Cannot include `Timestamp` in index, converted `Timestamp` to `seconds` and stored as `u64`
...
impl<'a> IndexList<Bid> for BidIndicies<'a> {
    fn get_indexes(&'_ self) -> Box<dyn Iterator<Item = &'_ dyn Index<Bid>> + '_> {
        let v: Vec<&dyn Index<Bid>> = vec![
            ...
            &self.bidder,
            &self.bid_id, // &self.bidder_expires_at,
        ];
        Box::new(v.into_iter())
    }
}
...
pub fn bids<'a>() -> IndexedMap<'a, BidKey, Bid, BidIndicies<'a>> {
    let indexes = BidIndicies {
        ...
        bid_id: MultiIndex::new(|_pk: &[u8], d: &Bid| d.bid_id, "bids", "bids__bid_id"), // bidder_expires_at: MultiIndex::new(
...
pub struct CollectionBid {
    pub collection_bid_id: CollectionBidId,
    pub collection: Addr,
    pub bidder: Addr,
    pub price_per_item: Uint128,
    pub finders_fee_bps: Option<u64>,
    pub quantity: u64, // pub expires_at: Timestamp,
}
...
pub type CollectionBidKey = (Addr, Addr, CollectionBidId);
/// Convenience collection bid key constructor
pub fn collection_bid_key(
    collection: &Addr,
    bidder: &Addr,
    collection_bid_id: CollectionBidId,
) -> CollectionBidKey {
    (collection.clone(), bidder.clone(), collection_bid_id)
}
...
pub fn collection_bids<'a>(
    ...
    let indexes = CollectionBidIndicies {
        ...
        collection_price: MultiIndex::new(
            |_pk: &[u8], d: &CollectionBid| (d.collection.clone(), d.price_per_item.u128()),
            "col_bids",
